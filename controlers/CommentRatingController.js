...
class CommentRatingController {
  static async addCommentRating(req, res) {
    ...
    const commentRating = comment_id
    ...
    if (comment_id) {
      ...
      if (commentRating) {
        if (commentRating.commentRating === rating) {
          ...
          if (rating) {
            CommentController.updateCommentRating(
              { commentLike: comment.commentLike - 1 },
              comment_id
            );
          } else {
            CommentController.updateCommentRating(
              { commentDisLike: comment.commentDisLike - 1 },
              comment_id
            );
          }
        } else {
      ...
      if (commentRating) {
        if (commentRating.commentRating === rating) {
          ...
          if (rating) {
            CommentAnwserController.updateCommentRating(
              { commentLike: commentanwser.commentLike - 1 },
              coment_anwser_id
            );
          } else {
            CommentAnwserController.updateCommentRating(
              { commentDisLike: commentanwser.commentDisLike - 1 },
              coment_anwser_id
            );
          }
        } else {
